...
const { buildClient } = require('./uazapi');
...
app.post('/api/instances/:id/export-analysis', async (req, res) => {
  try {
    ...
    // Busca todas as mensagens recentes por chat (limitando a quantidade para performance)
    const pageSize = 100;
    let offset = 0;
    const allChats = [];
    for (;;) {
    ...
    // Função para extrair timestamp numérico de uma mensagem
    const getTs = (m) => {
      const ts = m?.messageTimestamp || m?.timestamp || m?.wa_timestamp || m?.createdAt || m?.date || null;
      if (!ts) return null;
      // se for string numérica, converte
      if (typeof ts === 'string' && /^\d+$/.test(ts)) {
        const num = Number(ts);
        // se for em segundos (<1e12), multiplica por 1000
        return num < 10 ** 12 ? num * 1000 : num;
      }
      const n = Number(ts);
      if (Number.isFinite(n)) return n < 10 ** 12 ? n * 1000 : n;
      // tenta Date parse
      const d = new Date(ts);
      const ms = d.getTime();
    ...
    // Concatena todas as mensagens novas (após lastTs)
    let newMessages = [];
    ...
    for (const chat of allChats) {
      ...
      for (const m of msgs) {
        const ms = getTs(m);
        if (ms == null) continue;
        if (lastTs && ms <= new Date(lastTs).getTime()) continue;
        // Salva info
        newMessages.push({ chatId, msg: m, timestamp: ms });
        if (ms > maxTs) maxTs = ms;
      }
    }

    if (!newMessages.length) {
      // Nenhuma mensagem nova: retorna e não altera o timestamp
    ...
    // Constrói o texto de entrada para o ChatGPT
    const labelForMsg = (m) => (m?.fromMe === true || m?.sender?.fromMe === true || m?.me === true) ? 'Usuário' : 'Cliente';
    let transcript = '';
    for (const { chatId, msg } of newMessages) {
      const ts = msg?.messageTimestamp || msg?.timestamp || msg?.wa_timestamp || msg?.createdAt || msg?.date || '';
      const text = msg?.text || msg?.body || msg?.message || msg?.content?.text || msg?.content || JSON.stringify(msg);
      const who = labelForMsg(msg);
      transcript += `[${ts}] (${who}) ${chatId}: ${typeof text === 'string' ? text : JSON.stringify(text)}\n`;
    }

    // Define prompt base para o analista
    const systemPrompt =
      'Você é um assistente de análise de conversas da Luna. Receberá transcrições de ' +
      'interações de WhatsApp entre a assistente automatizada (Luna) e potenciais clientes. ' +
      'Seu papel é identificar oportunidades de melhoria na abordagem da Luna, sugerindo ' +
      'ajustes no tom, clareza, timing e conteúdo das mensagens para melhorar engajamento e taxa de resposta. ' +
      'Responda em português com um resumo sucinto e sugestões práticas.';
    const userPrompt =
      'Aqui está a transcrição das últimas mensagens trocadas entre a Luna e alguns clientes. ' +
      'Analise a conversa e indique pontos fortes, oportunidades de melhoria e sugestões de próximos passos.\n\n' +
      transcript;
    ...
    if (analysisEnabled) {
      ...
      try {
        ...
        suggestions = openaiResponse?.choices?.[0]?.message?.content || '';
      } catch (err) {
        const msg = err.response?.data?.error?.message || err.message || err.toString();
        console.error('Falha ao chamar API da OpenAI', msg);
        infoMessage = 'Não foi possível obter sugestões do serviço de análise. Mensagens armazenadas para próxima tentativa.';
      }
    } else {
