...
// Se o front chamar endpoints como /client-settings, /stats, /queue, etc. sem o prefixo /api,
// este middleware reescreve para /api/<endpoint>. Isso evita erros 404 quando o front
// acessa rotas sem o prefixo '/api'.
...
app.use((req, _res, next) => {
  // Extrai o primeiro segmento do caminho da requisição
  const seg = (req.path || '').replace(/^\/+/, '').split('/')[0];
  // Se for um endpoint conhecido e ainda não começar com '/api/', adiciona o prefixo
...
// Removido bloco duplicado de compat: reescrita de endpoints sem /api
 ...
 * Retorna a cota total de mensagens diárias, o número de mensagens já enviadas
 * no dia atual para o cliente, quantas ainda restam e a janela de envio.
 */
...
// Catch-all route: return 404 JSON instead of trying to serve a missing index.html
app.get('*', (req, res) => {
