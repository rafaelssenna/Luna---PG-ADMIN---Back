...
const ANALYSIS_OUTPUT_BUDGET = parseInt(process.env.OPENAI_OUTPUT_BUDGET || '1500', 10);
  ...
  try {
    ...
    if (analysisEnabled) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        // GPT‑5, GPT‑4o e modelos “omni” exigem max_completion_tokens; outros usam max_tokens
        const lowerModel = String(ANALYSIS_MODEL || '').toLowerCase();
        const useCompletionTokens = /gpt-5|gpt-4o|\bo[123]\b|\bomni/i.test(lowerModel);
        if (useCompletionTokens) {
          payload.max_completion_tokens = ANALYSIS_OUTPUT_BUDGET;
        } else {
          payload.max_tokens = ANALYSIS_OUTPUT_BUDGET;
        }
        // Temperatura só é definida para modelos que suportam; os reasoning models (gpt-5) ignoram
        if (!useCompletionTokens) {
          // Permite sobrepor via env OPENAI_TEMPERATURE (padrão 0.5)
          const temp = process.env.OPENAI_TEMPERATURE;
          const parsedTemp = temp !== undefined ? Number(temp) : 0.5;
        ...
        try {
          ...
          if (text) {
            ...
            appendLog(`✅ Lote ${i + 1} concluído (tamanho aproximado: ${chunks[i].length} chars).`);
          }
        } catch (err) {
