...
async function runLoopForClient(clientSlug, opts = {}) {
  ...
  try {
    ...
    try { getEmitter(clientSlug).emit('progress', { type: 'start', total: totalCount, at: new Date().toISOString() }); } catch {}
const settings = await getClientSettings(clientSlug);
    ...
    while (processed < batchSize) {
      ...
      processed++;
      try {
        const status = (typeof r !== 'undefined' && r && r.ok === false) ? 'error' : ((r && r.simulated) ? 'skipped' : 'success');
    ...
    try { getEmitter(clientSlug).emit('progress', { type: 'end', processed, at: new Date().toISOString() }); } catch {}
