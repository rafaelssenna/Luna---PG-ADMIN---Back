...
/* ======================  Compat: aceitar endpoints sem /api  ====================== */
// Se o front chamar /client-settings, /stats, /queue, etc., sem o prefixo /api,
// esta camada reescreve automaticamente para /api/<endpoint>.
const COMPAT_ENDPOINTS = new Set([
  'clients', 'client-settings', 'stats', 'queue', 'totals',
  'contacts', 'import', 'progress', 'loop', 'delete-client', 'healthz', 'quota'
]);
app.use((req, _res, next) => {
  const seg = (req.path || '').replace(/^\/+/,'').split('/')[0];
  if (seg && COMPAT_ENDPOINTS.has(seg) && !req.path.startsWith('/api/')) {
