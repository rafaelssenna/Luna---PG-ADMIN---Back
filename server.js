...
async function runLoopForClient(clientSlug, opts = {}) {
  ...
  try {
    ...
    // Processa em lotes
    // Cria emissor de progresso para este cliente se existir consumidor conectado
    const emitter = progressEmitters.get(clientSlug);
    while (processed < batchSize) {
      ...
      let result = { ok: true, simulated: !useIA };

      // Chama IA (se habilitado)
      if (useIA) {
        result = await runIAForContact({
          client: clientSlug,
          name,
          phone,
      ...
      // Emite evento de progresso para o front-end (se houver assinantes)
      if (emitter) {
        const status = !useIA
          ? 'skipped'
...
app.get('/api/progress', (req, res) => {
  ...
  // Força envio imediato dos cabeçalhos
  res.flushHeaders?.();
  // Mantém conexão ativa
  res.write('\n');
  // Obtém (ou cria) o emissor para este cliente
  let emitter = progressEmitters.get(clientSlug);
  if (!emitter) {
    emitter = new EventEmitter();
    progressEmitters.set(clientSlug, emitter);
  }
  // Define callback para envio de dados
  const sendData = (data) => {
    try {
      res.write(`data: ${JSON.stringify(data)}\n\n`);
    } catch (err) {
      // Falha ao escrever — encerramos a conexão
      try {
        res.end();
      } catch {}
    }
  };
  // Inscreve-se no evento 'progress'
  emitter.on('progress', sendData);
  // Remove inscrição quando conexão fecha
  req.on('close', () => {
    emitter.removeListener('progress', sendData);
  });
});
