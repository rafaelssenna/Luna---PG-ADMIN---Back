...
app.post('/api/instances/:id/export-analysis', async (req, res) => {
  try {
    ...
    if (!openaiKey) {
      return res.status(500).json({ error: 'Chave da API OpenAI não configurada (OPENAI_API_KEY).' });
    }
    ...
    try {
      ...
      openaiResponse = resp.data;
    } catch (err) {
      const msg = err.response?.data?.error?.message || err.message || err.toString();
      return res.status(500).json({ error: `Falha ao chamar API da OpenAI: ${msg}` });
    }

    const suggestions = openaiResponse?.choices?.[0]?.message?.content || '';
